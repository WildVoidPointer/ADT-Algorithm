...
void dec_to_other1(
    int num, int base, char res_buf[], 
    int *res_buff_offset, char place_map[]
) {
    int _rem = num % base;
    if (num >= base) {
        dec_to_other1(
            num / base, base, res_buf, 
            res_buff_offset, place_map
        );
    }
...
char* dec_to_other2(
    int num, int base, char place_map[]
) {
    if (num < base) {
       ...
       return _res_buf;
        ...
        char *_temp = dec_to_other2(
            num / base, base, place_map
        );

        char _cur_place_map[2] = { 
            place_map[num % base], '\0'
        };

        char *_res_buf = (char*) malloc (
            (strlen(_temp) + 2) * sizeof(char)
        );
