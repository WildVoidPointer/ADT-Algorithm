...
核心算法 (40 分)：**先写核心算法**，确保正确性

复杂度分析 (20 分)：**补充复杂度分析**（必写！）

代码规范 (15 分)：**添加简要注释**，一定要标注出题目中强调的关键字，**保持代码整洁**，变量命名合理

边界处理 (15 分)：**考虑 1 - 2 个边界情况**（空表、全 x 元素、单个元素、被修改元素处于数据结构的开头、结尾或中间等）

算法说明 (10 分)：不要长篇大论，不要伪代码混用，**文字描述即可**
...
- 考研算法题中 **除非特殊要求**，**不需要** 对数据结构进行 **手动初始化** 为零值或特定值
- 考研算法题中严禁出现其他系统调用，只能使用 `printf`  这类简单打印函数
- 如果题目未要求输出某条件下进行操作的提示，**不需要添加对应输出**，否则将会可能扣分
- 在考研题目中允许使用 `typedef int SeqListDataType;` 这类简单声明，但是 **不要引入函数指针和自体指针这种复杂操作**
...
- 考研算法代码题中，如果使用了快速幂等经典算法，需要 **简单说明该算法的核心思想**，但是注意 **不要在描述算法思想时描述代码逻辑**
- 考研算法的注释中，**不要出现详细的代码细节描述**，只需要进行 **基本功能的简单描述** 即可，如果该步骤是题目中的要求，则 **需要重点标注出来**
- 写完算法后的算法时间复杂度分析处，如果分多函数，需要分析每个部分的时间复杂度，即使算法的时间复杂度为常数次，也需要点明该算法的时间复杂度为 $O(1)$，最后进行整体算法时间复杂度综述
- 如果考研算法出现多函数模块，**优先保证每个模块的独立性，可能混淆的函数命名可以简短描述该函数功能**
